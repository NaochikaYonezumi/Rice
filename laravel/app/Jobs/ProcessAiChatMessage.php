    ...
    protected function buildPrompt(AiChatSession $session, AiChatMessage $assistant): string
    {
        ...
        if ($session->kind === AiChatSession::KIND_REPLY) {
            $prompt .= "【出力ルール (返信補助モード)】\n"
                ...
                . "\n"
                . "【返信本文を書く時の厳守ルール】\n"
                . "  以下を厳守してください. 1 つでも違反すると下流のメール本文挿入処理で誤動作します.\n"
                . "  1) 出力は「メール本文だけ」. 余計な前置き / 説明 / 解説 / 感想 / メタコメントは一切書かない.\n"
                . "  2) 「【reply】」「【返信】」「【返信案】」「【内容】」「**内容：**」 のような見出し / ラベルは絶対に書かない.\n"
                . "  3) 「件名：」「Subject:」「To:」「From:」「宛先:」「Cc:」 などのヘッダ行は絶対に書かない.\n"
                . "  4) 「---」「===」「***」 のような水平線 / 区切り線は絶対に書かない.\n"
                . "  5) 「以上をご参考に」「ご不明点があれば」「【カレンダーに記録】」 などの脚注は絶対に書かない.\n"
                . "  6) 署名 (会社名・住所・電話・FAX・Mail などの連絡先一覧) は絶対に書かない. 署名はユーザがプロフィール側で別途差し込みます.\n"
                . "  7) 書いて良いのは: 宛名 (例: 「○○様」) → 挨拶 → 本文段落 → 結語 (例: 「よろしくお願いいたします.」) まで.\n"
                . "  8) 自然な日本語の敬体 (です・ます調).\n"
